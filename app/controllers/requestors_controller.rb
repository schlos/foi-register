...
class RequestorsController < ApplicationController
  ...
  # GET /requestors.json
  def index
    @requestors = Requestor.paginate(:page => params[:page], :per_page => 100).all
    ...
    respond_to do |format|
      format.html {
        if request.env['HTTP_REFERER'] =~ /[?&]page=(\d+)$/
          redirect_to requestors_path(:is_admin => "admin", :page => $1)
        else
          redirect_to requestors_path(:is_admin => "admin")
        end
      }
      format.json { head :no_content }
    end
